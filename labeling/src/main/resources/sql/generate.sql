...
CREATE TABLE `generate`(
    ...
    `fields` VARCHAR(255) NOT NULL COMMENT '创建字段'
);
...
CREATE TABLE `?`(
    ...
    `?` VARCHAR(255) NOT NULL COMMENT '标注的值',
    `?2` VARCHAR(255) NOT NULL COMMENT '标注坐标',
    `?3` VARCHAR(255) NOT NULL COMMENT '标注的值(模型)',
    `?4` VARCHAR(255) NOT NULL COMMENT '标注的值(测试)'
);